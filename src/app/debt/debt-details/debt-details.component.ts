...
export class DebtDetailsComponent implements OnInit {
  ...
  ngOnInit() {
    if (this.debtService.debtActive == 'ongoing') {
      console.log('ongoing');
      // this.getdebtList();
    }
  }

  debtBtnClick(debtActive: 'ongoing'): void {
    this.debtService.debtActive = debtActive;
  }

  applydebt(data: any): void {
    if (data) {
      this.debtService.debtOrderData = data;
      this.router.navigate(['/debt/order']);
    }
  }
